...
export default {
  ...
  data() {
    return {
      ...
      category: '', //分类  下拉框数据绑定
      content: null, //内容     富文本数据绑定
      tags: [], //像后端请求所有标签数据，得到的数据存入数组中
  ...
  methods: {
    ...
    onEditorChange({ editor, html, text }) {
      ...
      //把当前富文本编辑器的内容实时的赋值给this.content
      this.content = html
    },
    generateUUID() {
    ...
    submit(){
        ...
        //创建article对象，即向后端请求发文章的接口需要传递的参数
        let article = {
            id:id,
            userId:this.user.id,
            title:this.title,
            category:this.category,
            summary:"文章摘要文章摘要文章摘要文章摘要文章摘要文章摘要文章摘要文章摘要",
